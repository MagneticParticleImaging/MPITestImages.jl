...
@testset "MPITestImages.jl" begin
    ...
    @testset "testimage_gen macro" begin
        MPITestImages.@testimage_gen function test_image_fun()
            return ones(4, 4)
        end
        ...
        @test_throws ErrorException(
            "The on-the-fly image `test_image_fun` has already been added.",
        ) MPITestImages.addOnTheFlyImage(:test_image_fun)
    ...
    @testset "TestImage struct" begin
        ...
        @test_throws ErrorException(
            "The given name `$name` did not match a known test image.",
        ) newTestImage = TestImage(name)
        ...
        @test data(newTestImage)[1, 6] == 1.0 &&
              isapprox(data(newTestImage)[end, end], 0.095, atol = 0.001)
    end
